...
function close_chat(usr_id){
    var confirmar=confirm("¿Estas seguro que quieres cerrar este chat?"); 
    if (confirmar){
        httu=http_obj();
        var usr_id='usr_id='+usr_id;
        httu.open('post','ucp_close_chat.php');
        httu.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
        httu.onreadystatechange=disp_usr;httu.send(usr_id);
        window.location='ucp_users.php';
        return false;
    } 
}

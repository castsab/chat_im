...
if (isset($_POST['ajx_sett'])) {
    $ajx_sett = $_POST['ajx_sett'];
} else {
    $ajx_sett = '0|0|0|0';
}
...
//echo '{bim_id = '.$bim_id.'} {bim_name = '.$bim_name.'} {title = '.$title.'}<br>';
...
if ($dbss['type'] == 'mysql' || $dbss['type'] == 'mysqli') {
    $llim = "LIMIT $from,10";
} else {
    $llim = "LIMIT 10 OFFSET $from";
}
if ($ajx_sett[0] == '1') {
    $order = 'usr_name ASC';
} else {
    $order = 'rtime DESC, usr_name ASC';
}
...
/* ---------------------------- */
//Se obtiene el rol que tiene el usuario
$query = 'SELECT role As rol FROM ' . $dbss['prfx'] . "_users WHERE usr_id=$bim_id";
$result = neutral_query($query);
$arrayResult = neutral_fetch_array($result);
$rol = $arrayResult['rol'];
/* ---------------------------- */

$query = 'INSERT INTO ' . $dbss['prfx'] . "_online VALUES($bim_id,'$bim_name','$ipaddr',$timestamp,$status,$rol)";
neutral_query($query);
...
if (isset($ajx_sett[2]) && $ajx_sett[2] == '1') {
    ...
    $cond = ($rol == 1)?'role in (0,1)':'role=1';
    
    /* ---------------------------- */
    //Solamente mustro los usuario del chat que esten disponibles
    //$query = 'SELECT count(*) FROM ' . $dbss['prfx'] . "_online WHERE ($timestamp-rtime)<$keep_time";
    $query = 'SELECT count(*) FROM ' . $dbss['prfx'] . "_online WHERE usr_id=$bim_id OR $cond And TIMESTAMPDIFF(HOUR,FROM_UNIXTIME(rtime),NOW()) <= 4 ";
    //echo 'query 1 : { '.$query.' }<br>';
    $total = neutral_query($query);
    $total = neutral_fetch_array($total);
    $total = $total[0];
    /* ---------------------------- */
    ...
    $query = 'SELECT * FROM ' . $dbss['prfx'] . "_online WHERE usr_id=$bim_id OR $cond And TIMESTAMPDIFF(HOUR,FROM_UNIXTIME(rtime),NOW()) <= 4 ORDER BY $order $llim";
    //echo 'query -> '.$query;
    ...
    if (neutral_num_rows($online) > 0) {
        ...
        while ($row = neutral_fetch_array($online)) {
            ...
            $lseen = $timestamp - $row['rtime'];
            $last_seen = $lang['seconds_ago'];
            if ($lseen > 90) {
                $lseen = round($lseen / 60);
                $last_seen = $lang['minutes_ago'];
            }
        ...
        include $skin_dir . '/templates/ucp_users_aj_footer.pxtm';
    } else {
        print '<div>' . $lang['no_users'] . '</div>';
    }
}
/* ---------------------- */
/* ---------------------- */
...
if (isset($ajx_sett[2]) && $ajx_sett[2] == '2') {
    ...
    if (neutral_num_rows($chats) > 0) {
        ...
        switch ($options[2]) {
            case 1:$format = 'h:i:s A';
                break;
            case 2:$format = 'Y-m-d H:i:s';
                break;
            case 3:$format = 'd.m.Y H:i:s';
                break;
            case 4:$format = 'm/d/Y h:i:s A';
                break;
        ...
        include $skin_dir . '/templates/ucp_chats_aj_footer.pxtm';
    } else {
        print '<div>' . $lang['no_chats'] . '</div>';
    }
}
/* ------------------ */
